    ...
    class Language_Model:
        ...
        def generate(self, context=None, n=20):
            ...
            if context is not None:
                ...
                if not self.chars:
                    norm_context = tuple(norm_context.split()[-(self.n):])
                else:
                    norm_context = tuple(norm_context[-(self.n):])
                
                # If context length exceeds or equals n, return the prefix of length n
                if not self.chars and len(context.split()) >= n:
                    return ' '.join(context.split()[:n])
            ...
            if context:
                combined_text = (original_context + ' ' + generated_text[len(original_context):]).strip()
                return combined_text[:n] if self.chars else ' '.join(combined_text.split()[:n])
            else:
                return generated_text[:n] if self.chars else ' '.join(generated_text.split()[:n])
